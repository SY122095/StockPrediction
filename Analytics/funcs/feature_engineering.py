"""テクニカル指標・特徴量エンジニアリング (Analytics notebook 用)

Application/backend/lib/features.py と機能的に同一 (DB非依存、DataFrameベース)。
notebookから `from funcs import feature_engineering as fe` として利用する想定。
"""
...
def add_supply_demand_features(
    ...
    各 df_* の想定列: date_utc, value (long形式, funcs/supply_demand_collector.py の出力)。
    """
