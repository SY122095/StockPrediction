"""センチメント指標取得 (Analytics notebook 用): 暗号資産 Fear&Greed Index / VIX特徴量"""
...
def compute_vix_sentiment(df_macro: pd.DataFrame, window: int = 60) -> pd.DataFrame:
    """stock_collector.download_macro() が返す DataFrame (vix列を含む) から
    VIX センチメント特徴量 (level/変化率/z-score) を計算する。"""
